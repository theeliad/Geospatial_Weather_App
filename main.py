...
def load_station_data():
    ...
    try:
        ...
        st.error("Error: `all_weather_stations.csv` not found. Make sure it's in the same directory as the app.")
        return pd.DataFrame()
...
if not all_stations_df.empty:
    ...
    if selected_station_name:
        ...
        folium.Marker(
            [lat, lon],
            popup=selected_station_data['Station Name'],
            tooltip=selected_station_data['Station Name']
        ).add_to(m)

        # Using the older 'width' argument instead of 'use_container_width'
        ...
        st.warning(f"No weather stations found in your CSV for the selected state: **{selected_state}**")# app.py (or main.py) - LAYOUT FIX
